...
@Api(value = "Crear", description = "Operacion para la creacion de deuda de aplicaciones por Pagatodo360")
public class CrearDeudaApp {
  
  private static final Logger logger = LoggerFactory.getLogger(CrearDeudaApp.class);
  ...
  @ApiOperation(value = "Crea deudas para aplicaciones ")
  @PostMapping("/deudaApp")
  public ResponseEntity<?> create(@Valid @RequestBody PagoRestApi data,
    ...
    String references = String.valueOf(new Date( ).getTime( ));
    try {
    ...
    recibo.setReference_number(data.getReference_number( ));
    recibo.setSuscripcion(data.getNombres( ) + " " + data.getApellido_paterno( ));
    ...
    recibo.setReference_number(references + "-4");
    // if (data.getNro_recibo( ) != 0) data.setNro_recibo(Long.valueOf(references));
    try {
